...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $sql_comment = "INSERT INTO `comments`(`username`,`comment`,`news_id`)VALUES('$usernameComment','$comment','$newsId')";
    $result_comment = mysqli_query($conn, $sql_comment);
    if ($result_comment) {
        $commentsuccess = "Your comment has been recorded!!";
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="container my-2">
        <div class="row p-3">
            ...
            include '_dbconnect.php';
            $news_id = $_GET['newsId'];
            $sql = "SELECT * FROM `approvednews` WHERE `news_id`='$news_id'";
            ...
            <div class="col-md">
                <div class="container p-3">
                    ...
                    if ($commentsuccess) {
                        ...
                        <strong>' . $commentsuccess . '</strong>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                      </div>';
                    }
                    ?>
                    <h2 class="text-left text-secondary">Comments:</h2>
                    <button class="btn btn-secondary my-2" data-bs-toggle="modal" data-bs-target="#exampleModal">Leave a
                        Comment</button>
                    <?php
                    if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] != true) {
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <h4 class="text-primary"> Please login to leave a comment!!</h4>
                                </div>
                    ...
                    <div class="container my-2">
                        ...
                        include '_dbconnect.php';
                        $newsID = $_GET['newsId'];
                        $commentshow_sql = "SELECT * FROM `comments` WHERE `news_id`='$newsID'";
                        $commentshow_result = mysqli_query($conn, $commentshow_sql);
                        $nocomments = true;
                        while ($commentshow_row = mysqli_fetch_assoc($commentshow_result)) {
                            $nocomments = false;
                            $comment_time = $commentshow_row['datetime'];
                            $commentcontent = $commentshow_row['comment'];
                            $datetimecomment = new DateTime($comment_time);
                            $formattedDatecomment = $datetimecomment->format('jS F Y');
                            $comment_user = $commentshow_row['username'];
                            $sqluserimage="SELECT * FROM `users` WHERE `username`='$comment_user'";
                            $userimageresult=mysqli_query($conn,$sqluserimage);
                            $rowimage=mysqli_fetch_assoc($userimageresult);
                            $profilePicture = $rowimage['profileimage'];
                        if ($profilePicture != NULL) {
                            ...
                            echo '<div class="d-flex align-items-center my-3">
                              <img src="'.$pic.'" width="10%" alt="..." class="mx-3">
                              <div class="flex-grow-1">
                              <p class="font-weight-bold my-0">' . $comment_user . ' at ' . $formattedDatecomment . '</p>
                               ' . $commentcontent . '
                              </div>
                            </div>';
                        }
                        if ($nocomments) {
                            echo '<div class="d-flex align-items-center my-3">
                              <div class="flex-grow-1">
                              <p class="font-weight-bold my-0">No Comments for this news at this time!!</p>
                              </div>
                            </div>';
                        }
                        ?>
