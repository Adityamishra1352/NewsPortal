...
<head>
    ...
    $userid=$_GET['userId'];
    $title="SELECT * FROM `users`WHERE `user_id`='$userid'";
    $fortitle=mysqli_query($conn,$title);
    $fortitlerow=mysqli_fetch_assoc($fortitle);
    $fname=$fortitlerow['firstname'];
    $lname=$fortitlerow['lastname'];
    ?>
...
<body>
    ...
    <div class="container mt-2 w-75">
        ...
        $profilePicture = $userrow['profileimage'];
        if ($profilePicture != NULL) {
            $pic = $profilePicture;
        } else {
            $pic = "images/userdefault.png";
        }
        echo '<div class="d-flex justify-content-center"><img src="' . $pic . '" alt="" class="w-50"></div>
        <h4 class="text-center my-2">' . $fname . ' ' . $lname . ' | ' . $location . '</h4><br>
        <hr>
        <p class="text-center">' . $bio . '</p><br><hr>
        <div class="container p-2 rounded" style="background-color:lightgray;">
            <h2 class="text-center" >Articles by ' . $fname . ' ' . $lname . '</h2>
            </div>';
        $userarticles = "SELECT * FROM `news` WHERE `user_name`='$username'";
        $userarticles_result = mysqli_query($conn, $userarticles);
        while ($userarticlerow = mysqli_fetch_assoc($userarticles_result)) {
            $title = $userarticlerow['title'];
            $news_id = $userarticlerow['news_id'];
            $description = $userarticlerow['description'];
            $newstype = $userarticlerow['newstype'];
            echo '
        <div class="d-flex align-items-center my-3">
            <img src="https://source.unsplash.com/100x100/?' . $newstype . '" width="10%" alt="..." class="m-3">
            <div class="flex-grow-1">
            <a class="font-weight-bold my-0 text-dark" href="news.php?newsId=' . $news_id . '"><b>' . $title . '</b></a>
            <p class="text-secondary">' . substr($description, 0, 50) . '...</p>
            <p class="text-primary">' . $newstype . '</p>
            </div>
        </div>';
        }
        ?>
